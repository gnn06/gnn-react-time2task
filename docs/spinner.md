...
## 5. Initialisation sans flash

Le store est pré-chargé synchroniquement depuis le localStorage (`preloadedState` dans `store.js`). Le premier render connaît déjà la session — plus de flash de l'écran de login au refresh.
